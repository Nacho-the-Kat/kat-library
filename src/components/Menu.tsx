        ...
        <Menu as="div" className="relative inline-block text-left lg:hidden">
          {({ open }) => (
            <>
              ...
              {open && (
                <div className="absolute left-0 mt-2 w-48 origin-top-left bg-white dark:bg-gray-800 shadow-lg ring-1 ring-black ring-opacity-5 rounded-md focus:outline-none">
                  {items.map((item, idx) => (
        ...
        <ul className="hidden lg:flex space-x-6">
          {items.map((item, idx) => (
            <li key={idx} className="relative group">
              <a
                href={item.href || "#"}
                className="text-gray-700 dark:text-white px-3 py-2 font-medium hover:text-primary-500"
              >
                {item.label}
                {item.children && <ChevronDownIcon className="w-4 h-4 inline ml-2 text-gray-500" />}
              </a>
              {item.children && (
                <div className="absolute left-0 mt-2 w-48 bg-white dark:bg-gray-800 shadow-lg rounded-md opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-opacity">
                  {item.children.map((subItem, subIdx) => (
                    <a
                      key={subIdx}
                      href={subItem.href || "#"}
