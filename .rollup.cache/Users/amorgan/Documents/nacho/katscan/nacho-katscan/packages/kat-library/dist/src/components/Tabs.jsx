      ...
      <Tab.Group selectedIndex={selectedIndex} onChange={setSelectedIndex}>
        <div className="relative">
          ...
          <Tab.List className="flex relative z-10">
            {tabs.map((tab, index) => (<Tab key={index} className={({ selected }) => `px-4 py-2 text-sm font-medium transition-colors duration-200 outline-none relative
                  ${selected
                ? "text-teal-500"
                ...
                {({ selected }) => (<div className="relative">
                    ...
                    {selected && (<div className="absolute left-0 right-0 bottom-[-2px] border-b-4 border-teal-400"></div>)}
                  </div>)}
              </Tab>))}
          </Tab.List>
        </div>
        ...
        <Tab.Panels>
          {tabs.map((tab, index) => (<Tab.Panel key={index} className="pt-4">
              {tab.content}
            </Tab.Panel>))}
        </Tab.Panels>
      </Tab.Group>
