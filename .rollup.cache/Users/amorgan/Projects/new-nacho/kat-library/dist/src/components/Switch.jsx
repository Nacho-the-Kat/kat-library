...
const SwitchComponent = ({ enabled, onChange, name, legend, icon }) => {
    return (<div className="flex items-center space-x-2 text-grayCustom">
      {icon && <span className="w-5 h-5 text-primary">{icon}</span>}
      <Switch checked={enabled} onChange={onChange} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors
